...
const parseAlert = alertXml => {
  ...
  return xml2js.parseStringPromise(alertXml, XML_PARSE_OPTIONS).then(alertJson => {
    ...
    const alert = common.mergeObjects(normalize(alertJson), { info: { area: {} } });

    // compute hash
    alert.hash = common.hashOf(alert);

    // normalize sent date
    alert.sent = !lodash.isEmpty(alert.sent) ? new Date(alert.sent) : alert.sent;

    // normalize onset date
    alert.info.onset = !lodash.isEmpty(alert.info.onset)
      ? new Date(alert.info.onset)
      : alert.info.onset;

    // normalize expires date
    alert.info.expires = !lodash.isEmpty(alert.info.expires)
      ? new Date(alert.info.expires)
      : alert.info.expires;

    // parse circle and polygon to geojson geometry
    if (!lodash.isEmpty(alert.info.area.polygon) || !lodash.isEmpty(alert.info.area.circle)) {
